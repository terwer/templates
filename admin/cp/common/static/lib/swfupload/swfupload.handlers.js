$(function () {
    ...
    $.swfUpLoadDefaults.property = {
        ...
        filesize: "2048", //文件大小
        btntext: "浏览...", //上传按钮的文字
        btnwidth: 112, //上传按钮的宽度
        btnheight: 30, //上传按钮的高度
    ...
    $.fn.InitSWFUpload = function (p) {
        ...
        var swfu = new SWFUpload({
            ...
            //button_image_url: '/Themes/default/base/images/btn_upload.png',
...
function uploadSuccess(file, serverData) {
    try {
        var tempstr = eval('(' + serverData + ')');
        var jsonstr = eval('(' + tempstr + ')');
        if (jsonstr.msg == '0') {
            var progressObj = $(this.customSettings.upload_target).children(".upload-progress");
            progressObj.children(".txt").html("上传失败。");
        }
        if (jsonstr.msg == '1') {
            //如果是单文件上传，则赋值相应的表单
            if (this.customSettings.button_action == SWFUpload.BUTTON_ACTION.SELECT_FILE) {
                $(this.customSettings.upload_target).siblings(".upload-path").val(jsonstr.msgbox);
                $(this.customSettings.upload_target).siblings(".upload-thumb").attr("src",jsonstr.msgbox);
            } else {
                var pathes = jsonstr.msgbox.split(",");
                var path = pathes[0];
                var thumb = pathes[1];
...
function addImage(targetObj, originalSrc, thumbSrc) {
    //插入到相册UL里面
    var newLi = $('<li>'
    + '<input type="hidden" name="'+targetObj[0].id+'" value="0|' + originalSrc + '|' + thumbSrc + '" />'
    + '<input type="hidden" name="' + targetObj[0].id + 'Remark" value="" />'
    ...
    + '</li>');
    newLi.appendTo(targetObj.next(".photo-list").children("ul"));
}
//设置相册封面
...
//设置图片描述
function setRemark(obj,boxid) {
    var parentObj = $(obj); //父对象

    //取得隐藏值
    var hidRemarkObj = parentObj.prevAll("input[name='"+boxid+"Remark']").eq(0); //取得隐藏值
    var remarkValueObj = $(obj).prevAll(".img-box").find(".remark i");
    ...
    var m = top.dialog({
        ...
        padding: 0,
        title: "图片描述",
        content: '<textarea id="ImageRemark" class="form-control"  style="margin:10px 0;font-size:12px;padding:3px;color:#000;border:1px #d2d2d2 solid;vertical-align:middle;width:300px;height:50px;">' + remarkValueObj.html() + '</textarea>',
        okValue: '批量描述',
        ok: function () {

            var remarkObj = $('#ImageRemark', parent.document);
            if (remarkObj.val() == "") {
                showTopMsg("总该写点什么吧？", "错误提示", "Error");
                return false;
            }
            parentObj.parent().parent().find("li input[name='"+boxid+"Remark']").val(remarkObj.val());
